...
@dataclass
class OpenAIEnvironmentDescriptionAgent:
    """OpenAI Responses API transport for the environment-description section."""
    ...
    def run(self, request: SectionRequest) -> AgentResult:
        if not self.settings.is_configured:
            raise OpenAIConfigurationError("חסרה הגדרת OpenAI. יש להגדיר OPENAI_API_KEY ו-OPENAI_MODEL.")
        ...
        try:
            ...
            raise RuntimeError(f"הקריאה לסוכן OpenAI נכשלה: {error}") from error
        output_text = str(getattr(response, "output_text", "") or "").strip()
        if not output_text:
            raise RuntimeError("הסוכן OpenAI לא החזיר טקסט.")
        return AgentResult(
            output_text=f"מודל: {self.settings.model}\n\n{output_text}",
            warnings=warnings,
            metadata={"agent": self.__class__.__name__, "mode": "openai", "model": self.settings.model or ""},
        )
    ...
    def _create_client(self) -> Any:
        ...
        try:
            ...
            raise OpenAIConfigurationError("חסרה ספריית OpenAI. יש להריץ pip install -e \".[dev]\".") from error
    ...
    def _build_content(self, request: SectionRequest) -> tuple[list[dict[str, str]], list[str]]:
        content: list[dict[str, str]] = [
            {
                "type": "input_text",
                "text": build_user_message(request.address, request.example, request.additional_request),
            }
        ]
        warnings: list[str] = []
    ...
    def _file_part(attachment: Attachment) -> tuple[dict[str, str] | None, str | None]:
        ...
        return {"type": "input_file", "filename": attachment.name, "file_data": f"data:{mime_type};base64,{encoded}"}, None
